		...
		unsigned long npages)
{
	...
	slot->vfn_dsm_state = kvzalloc(npages * sizeof(*slot->vfn_dsm_state), GFP_KERNEL);
	if (!slot->vfn_dsm_state)
		return -ENOMEM;

	//slot->rmap = kvm_kvzalloc(npages * sizeof(*slot->rmap));
	slot->rmap = kvzalloc(npages * sizeof(*slot->rmap), GFP_KERNEL);
	if (!slot->rmap) {
		ret = -ENOMEM;
	...
	slot->backup_rmap = kvzalloc(npages * sizeof(*slot->backup_rmap), GFP_KERNEL);
	if (!slot->backup_rmap) {
		ret = -ENOMEM;
		goto out_free_rmap;
	}
