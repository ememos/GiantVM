...
DEF("vcpu", HAS_ARG, QEMU_OPTION_vcpu,
	"-vcpu vcpunum=id,affinity=cpu\n"
	"-vcpu pin-all=on\n"
	"                set the vcpu affinity, pin vcpu with id=[id] to physical cpu with id=[cpu]\n"
	"                pin-all makes all vcpus are pinned on a cpu whose id is same with vcpu id.\n", QEMU_ARCH_ALL)
STEXI
@item -vcpu vcpunum=id,affinity=cpu
@item -vcpu pin-all
@findex -vcpu
set the vcpu affinity, pin vcpu with id=@samp{id} to physical cpu with id=@samp{cpu}
pin-all makes all vcpus are pinned on a cpu whose id is same with vcpu id.
ETEXI
